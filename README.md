...
$$\mathrm{Erosion}(I, B)(x, y) = \min_{(i, j) \in B} I(x + i, y + j)$$

* **Dilation (Genişletme):** Yerel maksimumu alır; nesne sınırlarını genişletir ve iç delikleri kapatır:

$$\mathrm{Dilation}(I, B)(x, y) = \max_{(i, j) \in B} I(x - i, y - j)$$

* **Opening (Açma):** $\mathrm{Dilate}(\mathrm{Erode}(I))$ — Küçük dış parazitleri yok eder.
* **Closing (Kapama):** $\mathrm{Erode}(\mathrm{Dilate}(I))$ — Nesne içindeki siyah çatlakları ve delikleri kapatır.
...
$$F(u, v) = \mathcal{F}_{\mathrm{col}}\left(\mathcal{F}_{\mathrm{row}}(I)\right)$$
